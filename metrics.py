...
def standard_accuracy(model: nn.Module, data: DataLoader):
    """
    computes the standard accuracy as well as the standard balanced accuracy for the model
    """
    pred_list, target_list = [], []
    for inputs, targets in batch_generator(data):
        pred_list.append(model.predict(inputs))
        target_list.append(targets)
        acc = accuracy(pred_list, target_list)
        balanced_acc = balanced_accuracy(pred_list, target_list)
    return {'acc': acc.item(), 'balanced_acc': balanced_acc.item()}


def robust_accuracy(model: DataModel, data: DataLoader, epsilon: float, latent_index: int):
    """
    computes the empirical robust accuracy using pgd for both the encoder and the classifier
    """
...
def smoothing_accuracy(model, epsilon, data: DataLoader, latent_index: int):
    """
    computes the accuracy using randomized smoothing
    """
    model = model.to(DEVICE)
    latent_pdg = SegmentPDG(
    ...
    for inputs, targets in batch_generator(data):
        ...
        for delta, latent, target in zip(deltas, latents, targets):
            ...
            pred = smooth.predict(latent, 20000, 0.001, batch_size=2000)
            if chat == target and radi > delta.item():
                num_certified += 1
            if pred == target:
                num_correct += 1
                
    return {'acc': num_correct/len(data.dataset), 'certified': num_certified/len(data.dataset)}
